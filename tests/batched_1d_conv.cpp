...
template <class T> ReferenceData<T> generate_data(int n, int batch) {
  ...
  for (int b = 0; b < batch; ++b) {
    for (int i = 0; i < n; ++i) {
      ...
      // y = 0;
      // double val = (b + 1) * sin(3 * k0 * x);
      // double dx_val = (b + 1) * 3 * k0 * cos(3 * k0 * x);
...
#define CHECK_VKFFT_RESULT(INPUT)                                              \
  ...
  CHECK_VKFFT_RESULT(
      perform_batched_1d_r2c_convolution_VkFFT<double>(vkGPU, 128, 3));
  CHECK_VKFFT_RESULT(
  ...
  CHECK_VKFFT_RESULT(
      perform_batched_1d_r2c_convolution_VkFFT<double>(vkGPU, 2048, 768));
